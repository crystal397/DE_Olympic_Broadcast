...
import json
from insert_data_to_bvb_db import insert_sport_event_query, insert_competitors_query, insert_venue_query, insert_period_scores_query
import logging
...
def send_data_kafka2db():
    conf = {
        ...
        'group.id': 'realtime_beachvolleyball', # logstash, 2
        'auto.offset.reset': 'latest',  # 'latest', 'earliest'
        'enable.auto.commit': True,  # 오프셋 자동 커밋 설정
        # 'session.timeout.msw': 10000,  # 세션 타임아웃 설정
    }
    ...
    while True:
        ...
        try:
            ...
            if sport_event_context.get('sport', {}).get('name') == '비치발리볼':
                insert_sport_event_query(data)
                insert_competitors_query(data)
                insert_venue_query(data)
                insert_period_scores_query(data)
            else:
                print(f"비치발리볼 데이터가 아닙니다. {sport_event_context.get('sport', {}).get('name')}입니다.")
        except KeyError:
            print("data 형식 error")
            continue
